...
DefRect RectangleItem::GetObject()
{
    ...
    return DefRect(tPoint.toPoint(),abs(polygon[1].x()-polygon[0].x()),abs(polygon[0].y()-polygon[3].y()));
}
...
DefRotateRect RectangleRItem::GetObject()
{
    ...
    return DefRotateRect(tPoint.toPoint(),Distance(polygon[0],polygon[1]),Distance(polygon[0],polygon[3]),angle);
}
...
{
    auto tPoint=this->mapToScene((polygon[0]+polygon[2])/2);
    return DefRotateEllipse(tPoint.toPoint(),Distance(polygon[0],polygon[1]),Distance(polygon[0],polygon[3]),angle);
}
...
DefCircle CircleItem::GetObject()
{
    ...
    return DefCircle(tPoint.toPoint(),Radius);
}
...
void ConcentricCircleItem::SetObject(DefConcentricCircle concentricCircle)
{
    ...
    RadiusMax=concentricCircle.rOuter;
    RadiusMin=concentricCircle.rInner>RadiusMax?RadiusMax:concentricCircle.rInner;
    UpDate();
}
DefConcentricCircle ConcentricCircleItem::GetObject()
{
    auto tPoint=this->mapToScene(Center);
    return DefConcentricCircle(tPoint.toPoint(),RadiusMax,RadiusMin);
}
...
void PieItem::SetObject(DefPie defPie)
{
    ...
    Radius=defPie.r;
    StartAng=defPie.angleStart;
    EndAng=defPie.angleEnd;
    UpDate();
}
DefPie PieItem::GetObject()
{
    return DefPie(this->mapToScene(Center).toPoint(),Radius,StartAng,EndAng);
}
void PieItem::Create(DefPie defPie)
{
    ...
    Radius=defPie.r;
    StartAng=defPie.angleStart;
    EndAng=defPie.angleEnd; //endAng>StartAng?StartAng:
    ControlList<<new ControlItem(this,QPointF(),0);
    ControlList<<new ControlItem(this,QPointF(),1);
    ControlList<<new ControlItem(this,QPointF(),2);
...
{
    ControlList<<new ControlItem(this,QPointF(),0);
    for(int i=0;i<defPolygon.lstPoint.size();i++)
    {
        polygon<<defPolygon.lstPoint[i];

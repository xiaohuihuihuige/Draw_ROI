...
QGView::QGView(QWidget *parent) : QGraphicsView(parent)
{
    ...
    addAction(new QAction(QStringLiteral("保存原图"),this));
    addAction(new QAction(QStringLiteral("保存截图"),this));
    setContextMenuPolicy(Qt::ActionsContextMenu);

    grayValue = new QGraphicsTextItem();
    grayValue->setFont(QFont("宋体", this->height()/15));
    grayValue->setVisible(true);
    grayValue->setPos(100,100);
    scene->addItem(grayValue);
...
void QGView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(X>-1&&X<ImageCatch.width()&&Y>-1&&Y<ImageCatch.height())
    {
        ...
        QString InfoVal=QString("X,Y(%1,%2)\nRGB(%3,%4,%5)").arg(QString::number((int)X),4,' ')
                .arg(QString::number((int)Y),4,' ')
                .arg(QString::number(R),3,' ')
                .arg(QString::number(G),3,' ')
                .arg(QString::number(B),3,' ')
                ;
        grayValue->setPlainText(InfoVal);
        grayValue->setPos(item);
...
void QGView::enterEvent(QEvent *pEvent)
{
    grayValue->setVisible(true);
    QGraphicsView::leaveEvent(pEvent);
}

void QGView::leaveEvent(QEvent *pEvent)
{
